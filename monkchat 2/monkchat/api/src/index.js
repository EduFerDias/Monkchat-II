...
app.post('/login', async(req, resp) => {
    ...
    let senhaCrypto = krypto.SHA256(senha).toString(krypto.enc.Base64)
...
app.post('/usuario', async (req, resp) => {
    try {
        ...
        let r = await db.tb_usuario.create({
            ...
            ds_senha: krypto.SHA256(usuParam.senha).toString(krypto.enc.Base64)
        })
        resp.send(r);
    } catch (e) {
        resp.send({ erro: 'Ocorreu um erro!'})
